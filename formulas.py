...
for row in range(2, max_row + 1):
    ...
    sheet.cell(row=row, column=col_lucro_total).value = formula_valor_total
    
    
linha_total = max_row + 2
sheet.cell(row=linha_total, column=col_name_produto, value='Totais Gerais')
sheet.merge_cells(start_row=linha_total, start_column=col_name_produto, end_row=linha_total, end_column=col_quantidade)

formula_lucro_total = f'SUM({get_column_letter(col_lucro_total)}2:{get_column_letter(col_lucro_total)}{max_row})'
sheet.cell(row=linha_total, column=col_lucro_total).value = formula_lucro_total

formula_lucro_total = f'SUM({get_column_letter(col_valor_total)}2:{get_column_letter(col_valor_total)}{max_row})'
sheet.cell(row=linha_total, column=col_valor_total).value = formula_valor_total
